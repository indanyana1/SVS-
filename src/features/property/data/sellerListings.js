...
const listingToRow = (l) => ({
	...
	street_address: l.streetAddress || null,
	...
	floor: l.floor || null,
	total_floors: l.totalFloors || null,
	age: l.age || null,
...
export const buildSellerListing = ({
	...
	return {
		...
		fullAddress: [streetAddress, suburb, location, city, province, postalCode, country]
			.map((s) => (s || '').trim())
			.filter(Boolean)
			.join(', '),
		...
		rating: 0,
		reviews: 0,
		availability: availability?.trim() || 'Available Now',
		facing: facing?.trim() || 'N/A',
		floor: floor?.trim() || '—',
