...
-- a date), and lets the booking modal flag exact date+time conflicts so
-- other buyers get steered to a free slot.
alter table if exists public.home_care_bookings add column if not exists booking_time text;

-- How long the service is expected to take, in minutes. Lets the booking
-- modal check real interval overlap against other bookings on the same date
-- instead of only catching an exact duplicate start time.
alter table if exists public.home_care_bookings add column if not exists duration_minutes integer not null default 60;
...
create table if not exists public.home_care_bookings (
  ...
  -- Requested date/time + which service/pricing option + free-form notes.
  booking_date text,
  booking_time text,
  duration_minutes integer not null default 60,
  service_label text,
...
-- Speeds up "what's already booked for this provider on this date" lookups
-- used by the booking modal's exact-time conflict check.
create index if not exists home_care_bookings_provider_date_idx
  on public.home_care_bookings (provider_id, booking_date);
